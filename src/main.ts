  ...
  private async announceOnce(): Promise<void> {
    ...
    try {
      ...
      await apstore.announceUpdate({
        ...
        version: this.manifest.version,
        summary: 'В настройке доступов появились две новые вкладки. «Запросы» — люди, которые нажали в приложении кнопку «Запросить доступ»: можно сразу выдать нужный уровень или отказать, человек узнает о решении письмом. «Внешние гости» — выдача временного доступа тому, у кого нет нашей рабочей почты: указываете адрес, приложение, уровень и срок, человеку уходит ссылка для входа, а по окончании срока доступ гаснет сам.',
      });
